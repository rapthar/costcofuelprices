      ...
      {stations.map(station => (
        ...
        >
          <div className="flex justify-between items-start">
            <div>
              <Link 
                to={window.location.pathname.startsWith('/canada')
                  ? `/station/canada/costco-gas-in-${station.City.toLowerCase()}-${station.Address.toLowerCase()}`.replace(/\s+/g, '-')
                  : `/station/us/${station.Title.toLowerCase().replace(/\s+/g, '-')}`}
