...
import { MapPin, Phone, Clock, Calendar, Info, ChevronRight, Building2, Globe2, Thermometer } from 'lucide-react';
...
const CanadaStationPage = () => {
  ...
  React.useEffect(() => {
    if (station) {
      // Fetch weather data for the station's location
      fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${station.Latitude}&lon=${station.Longitude}&units=metric&appid=${process.env.REACT_APP_WEATHER_API_KEY}`)
        .then(res => res.json())
  ...
  // Find nearby stations (within 80 kilometers)
  const nearbyStations = React.useMemo(() => {
    if (!station) return [];

    return canadaStations[0]
      .filter(s => {
        ...
        return distance <= 80; // 80 kilometers ≈ 50 miles
      })
      .slice(0, 3);
  }, [station]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <p className="text-2xl font-bold text-green-600">{formatCADPrice(parseFloat(station.Regular))}</p>
              </div>
              <div className="bg-gray-50 rounded-lg p-4">
                <p className="text-sm text-gray-600">Premium</p>
                <p className="text-2xl font-bold text-green-600">{formatCADPrice(parseFloat(station.Premium))}</p>
              </div>
              <div className="bg-gray-50 rounded-lg p-4">
                <p className="text-sm text-gray-600">Diesel</p>
                <p className="text-2xl font-bold text-green-600">{formatCADPrice(parseFloat(station.Diesel))}</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              {weather && (
                <div className="flex items-start gap-3">
                  <Thermometer className="w-5 h-5 text-gray-400 mt-1" />
                  <div>
                    <p className="font-medium text-gray-900">Current Weather</p>
          ...
          <div className="mt-8 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            ...
            <PriceChart station={station} isCanada={true} />
          </div>
        </div>
        ...
        <div className="lg:col-span-1">
          ...
          {nearbyStations.length > 0 && (
            <div className="mt-8 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <NearbyStations stations={nearbyStations} isCanada={true} />
            </div>
          )}
        </div>
      </div>
...
export default CanadaStationPage;